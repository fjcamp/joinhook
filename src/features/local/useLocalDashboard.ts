...
export function useLocalDashboard(businessId?: string) {
  const [dashboard, setDashboard] = useState<LocalDashboard | null>(null);
  const [runtime, setRuntime] = useState<RuntimeState>({ status: 'loading', online: true, locationPermission: 'unknown', lastUpdated: null });

  const load = useCallback(async () => {
    const cached = businessId ? null : readCachedDashboard();
    if (cached) {
      setDashboard(cached);
    ...
    try {
      const base = await localDataGateway.getDashboard(businessId);
      const { location, permission } = await resolveLocation();
      let weather = base.weather;
      let status: RuntimeState['status'] = 'ready';
      try { weather = await fetchWeather(location); } catch { status = cached ? 'stale' : 'degraded'; }
      const next: LocalDashboard = { ...base, location, weather, updatedAt: new Date().toISOString() };
      setDashboard(next);
      if (!businessId) writeCachedDashboard(next);
      setRuntime({ status, online: typeof navigator === 'undefined' ? true : navigator.onLine, locationPermission: permission, lastUpdated: next.updatedAt });
    } catch {
