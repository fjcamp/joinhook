...
import { EGOBadge, EGOEmpty, EGOField, EGOIcon, EGOModal, EGOStatCard } from '@/features/cge/components';
import { createBlankState, createDemoState, downloadBackup, exportInventoryCsv, loadState, makeId, normalizeEGOState, resetState, saveState } from '@/features/cge/storage';
import { EGOState, EGOView, Product, ProductUnit, Supplier, WasteReason } from '@/features/cge/types';
...
const views: { key: EGOView; label: string; caption: string }[] = [
...
export default function EGOApp() {
    const [state, setState] = useState<EGOState | null>(null);
    const [view, setView] = useState<EGOView>('resumen');
    ...
    if (!state) return <div className="cge-loading"><span>EGO</span><p>Preparando Estado de Gastos Operacionales…</p></div>;

    const mutate = (recipe: (draft: EGOState) => EGOState) => setState((current) => current ? recipe(current) : current);
    ...
    const importBackup = async (file?: File) => {
        if (!file) return;
        try {
            const parsed = normalizeEGOState(JSON.parse(await file.text()), 'real');
            ...
            <section className="cge-main">
                ...
                {view === 'resumen' && <>
                    ...
                    <section className="cge-panel cge-suggestions">
                        ...
                        {suggestions.length ? <div className="cge-suggestion-list">{suggestions.slice(0, 6).map((item) => <div key={item.product.id}><div className="cge-product-symbol">{item.product.name.slice(0, 2).toUpperCase()}</div><div><strong>{item.product.name}</strong><small>Actual {decimal.format(item.product.stock)} · mínimo {decimal.format(item.product.minStock)} {item.product.unit}</small></div><div><small>Sugerencia</small><strong>{decimal.format(item.quantity)} {item.product.unit}</strong></div><div><small>Estimado</small><strong>{money.format(item.estimatedCost)}</strong></div><button type="button" onClick={() => openPurchase(item.product.id, item.quantity)}>Registrar</button></div>)}</div> : <EGOEmpty title="No necesitas reponer según mínimos" text="Cuando un producto llegue a su stock mínimo aparecerá aquí." />}
                    </section>

                    <section className="cge-quick-grid"><button type="button" onClick={() => setProductModal({ open: true })}><span><EGOIcon name="inventario" /></span><div><strong>Nuevo producto</strong><small>Agregar al inventario</small></div><EGOIcon name="chevron" /></button><button type="button" onClick={() => setSupplierOpen(true)}><span><EGOIcon name="proveedores" /></span><div><strong>Nuevo proveedor</strong><small>Guardar contacto</small></div><EGOIcon name="chevron" /></button><button type="button" onClick={() => csvRef.current?.click()}><span>CSV</span><div><strong>Cargar inventario</strong><small>Importar desde Excel/Sheets</small></div><EGOIcon name="chevron" /></button></section>
                </>}

                {view === 'inventario' && <section className="cge-view"><div className="cge-view-tools"><div className="cge-search"><EGOIcon name="search"/><input placeholder="Buscar producto o categoría…" value={search} onChange={(event) => setSearch(event.target.value)} /></div><div className="cge-tool-actions"><button className="cge-btn" type="button" onClick={() => csvRef.current?.click()}>Importar CSV</button><button className="cge-btn cge-btn-primary" type="button" onClick={() => setProductModal({ open: true })}><EGOIcon name="plus"/> Nuevo producto</button></div></div><div className="cge-table-wrap"><table className="cge-table"><thead><tr><th>Producto</th><th>Categoría</th><th>Stock</th><th>Mínimo</th><th>Costo unit.</th><th>Valor</th><th>Estado</th><th /></tr></thead><tbody>{filteredProducts.map((item) => <tr key={item.id}><td><strong>{item.name}</strong><small>{suppliers.find((s) => s.id === item.supplierId)?.name || 'Sin proveedor'}</small></td><td>{item.category}</td><td>{decimal.format(item.stock)} {item.unit}</td><td>{decimal.format(item.minStock)} {item.unit}</td><td>{money.format(item.unitCost)}</td><td><strong>{money.format(item.stock * item.unitCost)}</strong></td><td><EGOBadge tone={item.stock === 0 ? 'danger' : item.stock <= item.minStock ? 'warn' : 'good'}>{item.stock === 0 ? 'Sin stock' : item.stock <= item.minStock ? 'Revisar' : 'Correcto'}</EGOBadge></td><td><div className="cge-row-actions"><button type="button" onClick={() => setAdjustProduct(item)}>Ajustar</button><button type="button" onClick={() => setProductModal({ open: true, product: item })}>Editar</button><button type="button" className="danger" onClick={() => deleteProduct(item)}>Eliminar</button></div></td></tr>)}</tbody></table>{!filteredProducts.length && <EGOEmpty title="No encontré productos" text="Prueba con otra búsqueda o crea uno nuevo." />}</div></section>}

                {view === 'compras' && <section className="cge-view"><div className="cge-view-tools"><div><span className="cge-kicker">Entradas de mercadería</span><h2>Compras registradas</h2></div><button className="cge-btn cge-btn-primary" type="button" onClick={() => openPurchase()}><EGOIcon name="plus"/> Registrar compra</button></div>{state.purchases.length ? <div className="cge-card-list">{state.purchases.map((item) => { const product = products.find((p) => p.id === item.productId); return <article key={item.id}><div className="cge-product-symbol">{product?.name.slice(0,2).toUpperCase() || '—'}</div><div><strong>{product?.name || 'Producto eliminado'}</strong><small>{suppliers.find((s) => s.id === item.supplierId)?.name || 'Compra sin proveedor'}</small></div><div><small>Cantidad</small><strong>{decimal.format(item.quantity)} {product?.unit}</strong></div><div><small>Costo unitario</small><strong>{money.format(item.unitCost)}</strong></div><div><small>Total</small><strong>{money.format(item.quantity * item.unitCost)}</strong></div><time>{dateFmt.format(new Date(item.date + 'T12:00:00'))}</time></article>})}</div> : <EGOEmpty title="Aún no has registrado compras" text="La primera compra actualizará automáticamente el stock del producto." />}</section>}

                {view === 'mermas' && <section className="cge-view"><div className="cge-view-tools"><div><span className="cge-kicker">Pérdidas controladas</span><h2>Mermas registradas</h2></div><button className="cge-btn cge-btn-clay" type="button" onClick={() => setWasteOpen(true)}><EGOIcon name="plus"/> Registrar merma</button></div>{state.wastes.length ? <div className="cge-card-list">{state.wastes.map((item) => { const product = products.find((p) => p.id === item.productId); return <article key={item.id}><div className="cge-product-symbol clay">{product?.name.slice(0,2).toUpperCase() || '—'}</div><div><strong>{product?.name || 'Producto eliminado'}</strong><small>{item.reason}</small></div><div><small>Cantidad</small><strong>{decimal.format(item.quantity)} {product?.unit}</strong></div><div><small>Costo estimado</small><strong>{money.format(item.quantity * (product?.unitCost || 0))}</strong></div><div className="cge-notes"><small>{item.notes || 'Sin observaciones'}</small></div><time>{dateFmt.format(new Date(item.date + 'T12:00:00'))}</time></article>})}</div> : <EGOEmpty title="Todavía no hay mermas" text="Registrarlas ayuda a entender dónde se pierde producto y dinero." />}</section>}

                {view === 'proveedores' && <section className="cge-view"><div className="cge-view-tools"><div><span className="cge-kicker">Red de abastecimiento</span><h2>Proveedores</h2></div><button className="cge-btn cge-btn-primary" type="button" onClick={() => setSupplierOpen(true)}><EGOIcon name="plus"/> Nuevo proveedor</button></div>{suppliers.length ? <div className="cge-supplier-grid">{suppliers.map((item) => <article className="cge-panel" key={item.id}><div className="cge-supplier-head"><span>{item.name.slice(0,2).toUpperCase()}</span><EGOBadge>{products.filter((p) => p.supplierId === item.id).length} productos</EGOBadge></div><h3>{item.name}</h3><p>{item.contactName || 'Sin persona de contacto'}</p><div className="cge-supplier-data"><span><small>Teléfono</small><strong>{item.phone || '—'}</strong></span><span><small>Correo</small><strong>{item.email || '—'}</strong></span></div></article>)}</div> : <EGOEmpty title="Sin proveedores todavía" text="Agrega proveedores para asociarlos a productos y compras." />}</section>}

                {view === 'respaldo' && <section className="cge-view"><div className="cge-backup-hero cge-panel"><div><span className="cge-kicker">Tus datos son tuyos</span><h2>Respalda antes de cambiar de dispositivo.</h2><p>Esta beta guarda todo localmente en este navegador. Puedes descargar un respaldo completo, exportar/importar inventario por CSV o recuperar un archivo anterior.</p></div><div className="cge-backup-mark"><EGOIcon name="respaldo"/></div></div><div className="cge-backup-grid"><button type="button" onClick={() => downloadBackup(state)}><span><EGOIcon name="respaldo"/></span><strong>Descargar respaldo</strong><small>Productos, compras, mermas y proveedores en JSON.</small></button><button type="button" onClick={() => exportInventoryCsv(state)}><span>CSV</span><strong>Exportar inventario</strong><small>Archivo compatible con Excel y Google Sheets.</small></button><button type="button" onClick={() => csvRef.current?.click()}><span>↥</span><strong>Importar inventario CSV</strong><small>Agrega o actualiza productos por nombre.</small></button><button type="button" onClick={() => backupRef.current?.click()}><span>↥</span><strong>Restaurar respaldo</strong><small>Reemplaza los datos actuales con un respaldo válido.</small></button><button type="button" className="danger" onClick={() => { if (window.confirm('¿Volver a los datos de demostración? Se perderán los datos actuales de este navegador.')) { resetState(); const demo = createDemoState(); setState({ ...demo, onboardingCompleted: true, businessName: state.businessName || 'Mi negocio gastronómico' }); notify('Demo reiniciada.'); } }}><span>↺</span><strong>Reiniciar demo</strong><small>Volver al conjunto de datos inicial.</small></button></div><input ref={backupRef} hidden type="file" accept="application/json,.json" onChange={(event) => importBackup(event.target.files?.[0])}/><div className="cge-privacy-note"><EGOIcon name="alert"/><div><strong>Privacidad de la beta</strong><p>No se está enviando información del negocio a un servidor de JoinHook. Si luego activamos cuentas y sincronización, la arquitectura de seguridad y consentimiento se implementará antes de almacenar datos en la nube.</p></div></div></section>}
            </section>
        </main>

        <input ref={csvRef} hidden type="file" accept="text/csv,.csv" onChange={(event) => importInventoryCsv(event.target.files?.[0])} />

        {onboardingOpen && <EGOModal title="Prepara tu espacio" eyebrow="Primer inicio · 1 minuto" onClose={() => setOnboardingOpen(false)}><div className="cge-onboarding"><p>Estado de Gastos Operacionales puede abrir con datos de ejemplo para explorar o completamente en blanco para comenzar con tu negocio.</p><EGOField label="Nombre del negocio"><input value={onboardingName} onChange={(event) => setOnboardingName(event.target.value)} placeholder="Ej. Cafetería del Lago" autoFocus /></EGOField><div className="cge-onboarding-options"><button type="button" onClick={() => finishOnboarding(false)}><span>Explorar</span><strong>Usar datos de ejemplo</strong><small>Ideal para conocer el flujo sin cargar información todavía.</small></button><button type="button" className="primary" onClick={() => finishOnboarding(true)}><span>Comenzar</span><strong>Crear espacio en blanco</strong><small>Para registrar desde cero tus productos y proveedores.</small></button></div><p className="cge-onboarding-note">En ambos casos los datos permanecen únicamente en este dispositivo durante esta beta.</p></div></EGOModal>}

        {productModal.open && <EGOModal title={productModal.product ? 'Editar producto' : 'Nuevo producto'} eyebrow="Inventario" onClose={() => setProductModal({ open: false })}><form className="cge-form" onSubmit={saveProduct} key={productModal.product?.id || 'new'}>{(() => { const defaults = productFormDefaults(productModal.product); return <><EGOField label="Nombre"><input name="name" required defaultValue={defaults.name} placeholder="Ej. Harina" autoFocus /></EGOField><div className="cge-form-grid"><EGOField label="Categoría"><input name="category" defaultValue={defaults.category} placeholder="Secos, lácteos…" /></EGOField><EGOField label="Unidad"><select name="unit" defaultValue={defaults.unit}>{units.map((unit) => <option key={unit}>{unit}</option>)}</select></EGOField></div>{productModal.product ? <div className="cge-stock-readonly"><div><small>Stock actual</small><strong>{decimal.format(productModal.product.stock)} {productModal.product.unit}</strong></div><button type="button" onClick={() => { setProductModal({ open: false }); setAdjustProduct(productModal.product || null); }}>Ajustar stock</button></div> : <EGOField label="Stock inicial"><input name="stock" type="number" min="0" step="0.01" required defaultValue={defaults.stock}/></EGOField>}<div className="cge-form-grid"><EGOField label="Stock mínimo"><input name="minStock" type="number" min="0" step="0.01" required defaultValue={defaults.minStock}/></EGOField><EGOField label="Costo unitario"><input name="unitCost" type="number" min="0" step="1" required defaultValue={defaults.unitCost}/></EGOField></div><EGOField label="Proveedor"><select name="supplierId" defaultValue={defaults.supplierId}><option value="">Sin proveedor</option>{suppliers.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></EGOField><div className="cge-form-actions"><button className="cge-btn" type="button" onClick={() => setProductModal({ open: false })}>Cancelar</button><button className="cge-btn cge-btn-primary" type="submit">Guardar producto</button></div></>; })()}</form></EGOModal>}

        {purchaseOpen && <EGOModal title="Registrar compra" eyebrow="Entrada de mercadería" onClose={() => { setPurchaseOpen(false); setPurchaseDraft({}); }}><form className="cge-form" onSubmit={registerPurchase} key={`${purchaseDraft.productId || 'new'}-${purchaseDraft.quantity || 0}`}><EGOField label="Producto"><select name="productId" required defaultValue={purchaseDraft.productId || ''}><option value="" disabled>Selecciona un producto</option>{products.map((item) => <option value={item.id} key={item.id}>{item.name} · {decimal.format(item.stock)} {item.unit}</option>)}</select></EGOField><div className="cge-form-grid"><EGOField label="Cantidad"><input name="quantity" type="number" min="0.01" step="0.01" required defaultValue={purchaseDraft.quantity || ''} /></EGOField><EGOField label="Costo unitario"><input name="unitCost" type="number" min="0" step="1" required defaultValue={selectedPurchaseProduct?.unitCost || ''} /></EGOField></div><div className="cge-form-grid"><EGOField label="Proveedor"><select name="supplierId" defaultValue={selectedPurchaseProduct?.supplierId || ''}><option value="">Usar proveedor del producto</option>{suppliers.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></EGOField><EGOField label="Fecha"><input name="date" type="date" defaultValue={today()} required /></EGOField></div><EGOField label="Nota opcional"><textarea name="notes" rows={3} placeholder="Factura, condición, observación…" /></EGOField><div className="cge-form-actions"><button className="cge-btn" type="button" onClick={() => { setPurchaseOpen(false); setPurchaseDraft({}); }}>Cancelar</button><button className="cge-btn cge-btn-primary" type="submit">Registrar y sumar stock</button></div></form></EGOModal>}

        {wasteOpen && <EGOModal title="Registrar merma" eyebrow="Salida por pérdida" onClose={() => setWasteOpen(false)}><form className="cge-form" onSubmit={registerWaste}><EGOField label="Producto"><select name="productId" required defaultValue=""><option value="" disabled>Selecciona un producto</option>{products.filter((item) => item.stock > 0).map((item) => <option value={item.id} key={item.id}>{item.name} · disponible {decimal.format(item.stock)} {item.unit}</option>)}</select></EGOField><div className="cge-form-grid"><EGOField label="Cantidad"><input name="quantity" type="number" min="0.01" step="0.01" required /></EGOField><EGOField label="Causa"><select name="reason" defaultValue="Preparación">{wasteReasons.map((reason) => <option key={reason}>{reason}</option>)}</select></EGOField></div><EGOField label="Fecha"><input name="date" type="date" defaultValue={today()} required /></EGOField><EGOField label="Observación"><textarea name="notes" rows={3} placeholder="Qué ocurrió y cómo podría evitarse…" /></EGOField><div className="cge-form-actions"><button className="cge-btn" type="button" onClick={() => setWasteOpen(false)}>Cancelar</button><button className="cge-btn cge-btn-clay" type="submit">Registrar y descontar stock</button></div></form></EGOModal>}

        {adjustProduct && <EGOModal title={`Ajustar ${adjustProduct.name}`} eyebrow="Corrección de inventario" onClose={() => setAdjustProduct(null)}><form className="cge-form" onSubmit={registerAdjustment}><div className="cge-stock-readonly"><div><small>Stock registrado</small><strong>{decimal.format(adjustProduct.stock)} {adjustProduct.unit}</strong></div><EGOBadge tone={adjustProduct.stock <= adjustProduct.minStock ? 'warn' : 'good'}>{adjustProduct.stock <= adjustProduct.minStock ? 'Bajo mínimo' : 'Correcto'}</EGOBadge></div><EGOField label="Nuevo stock"><input name="newStock" type="number" min="0" step="0.01" required defaultValue={adjustProduct.stock} autoFocus /></EGOField><EGOField label="Motivo del ajuste" hint="Quedará guardado en el historial."><textarea name="note" rows={3} required placeholder="Ej. Conteo físico, corrección de registro…" /></EGOField><div className="cge-form-actions"><button className="cge-btn" type="button" onClick={() => setAdjustProduct(null)}>Cancelar</button><button className="cge-btn cge-btn-primary" type="submit">Guardar ajuste</button></div></form></EGOModal>}

        {supplierOpen && <EGOModal title="Nuevo proveedor" eyebrow="Abastecimiento" onClose={() => setSupplierOpen(false)}><form className="cge-form" onSubmit={saveSupplier}><EGOField label="Nombre comercial"><input name="name" required autoFocus placeholder="Ej. Distribuidora Sur" /></EGOField><EGOField label="Persona de contacto"><input name="contactName" placeholder="Nombre y apellido" /></EGOField><div className="cge-form-grid"><EGOField label="Teléfono"><input name="phone" placeholder="+56 9…" /></EGOField><EGOField label="Correo"><input name="email" type="email" placeholder="ventas@…" /></EGOField></div><EGOField label="Notas"><textarea name="notes" rows={3} placeholder="Días de despacho, mínimo de compra…" /></EGOField><div className="cge-form-actions"><button className="cge-btn" type="button" onClick={() => setSupplierOpen(false)}>Cancelar</button><button className="cge-btn cge-btn-primary" type="submit">Guardar proveedor</button></div></form></EGOModal>}
    </>;
}
