...
export default function Home() {
  ...
  async function checkout() {
    ...
    const payload = { idempotencyKey, totalMinor: total, items: cart.map(x => ({ productId: x.id, quantity: x.quantity, unitMinor: x.priceMinor })) };
    try {
  ...
  return <main className="shell">
    ...
    <section className="content">
      {active === 'Inicio' && <div className="grid">
        <article className="card wide"><div className="eyebrow">BUSINESS OS · MVP</div><h1>Centro operacional</h1><p className="muted">Una sola operación: venta → caja → trazabilidad → respaldo. El cliente puede seguir trabajando localmente cuando la red falla.</p><div className="actions"><button onClick={() => setActive('POS / Caja')}>Abrir POS</button><button className="secondary" onClick={() => setActive('Bodega')}>Ver bodega</button></div></article>
        ...
        <article className="card"><h2>Respaldo</h2><div className="metric small">{cloud ? 'NUBE' : 'LOCAL'}</div><p className="muted">Datos pendientes se mantienen en cola local.</p></article>
