...
export async function GET() {
  if (!sql || !tenantId) return NextResponse.json({ cloud: false, sessions: [] });
...
export async function POST(request: Request) {
  ...
  if (action === 'close') {
    ...
    await sql`
      INSERT INTO audit.events (tenant_id, event_type, aggregate_type, aggregate_id, payload)
      VALUES (${tenantId}, 'CASH_SESSION_CLOSED', 'CASH_SESSION', ${rows[0].id}, ${JSON.stringify({ countedAmount })}::jsonb)
    `;
